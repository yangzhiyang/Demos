...
export default class SelectPlus extends PureComponent {
  constructor(props) {
    super(props);
    ...
    // 下拉菜单项行高
    this.ITEM_HEIGHT = ITEM_HEIGHT_SIZES[props.size || "default"];
    // 可视区 dom 高度
  ...
  onScroll = () => this.throttleScrollByHeight(this.onScrollReal);
  ...
  throttleScrollByHeight = () => {
    this.scrollTop = this.scrollEle.scrollTop;
    // 滚动的距离
    const delta = Math.abs(this.prevScrollTop - this.scrollTop);
  ...
  getStartAndEndIndex = () => {
    ...
    const firstItemIndex = Number((this.scrollTop / this.ITEM_HEIGHT).toFixed(0));

    const startIndex =
      firstItemIndex - ITEM_ELEMENT_NUMBER < 0
        ? 0
        : firstItemIndex - ITEM_ELEMENT_NUMBER / 2;
    const endIndex = firstItemIndex + ITEM_ELEMENT_NUMBER;
